...
for (const file of htmlFiles) {
  ...
  if (!isNoindex) {
    ...
    if (isCatalogDetail && !/<a\b[^>]*href=["']\/editorial\/["'][^>]*>(?:AWB|Answer with Books)<\/a>/i.test(html)) {
      fail(`${path}: catalog detail page has no visible linked editor byline`);
    }
    if (isAnswerDetail && !/<section\b[^>]*\bdata-reading-content\b[^>]*>/i.test(html)) {
      fail(`${path}: guide reading content is not present as an open section`);
    }
