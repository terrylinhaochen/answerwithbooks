...
export const availableTools = [
  { id: 'github-leads', name: 'GitHub lead research', provider: 'GitHub', kind: 'API', description: 'Find relevant builders and projects, with evidence behind each lead.', guide: '/guides/github-lead-research/', example: 'Find developers building inference infrastructure who could benefit from our managed model-serving API. Separate project fit from buying intent.' },
...
export function getAgentSetup(id, origin) {
  ...
  return {
    ...
    note: id === 'cursor' ? 'Use the skill in Cursor’s agent chat. An AWB marketplace plugin is not available yet.'
      : ['claude-ai', 'grok-bot'].includes(id) ? 'Cloud agents need a reachable HTTPS API and approved access. A local Mac API cannot be reached from their cloud computer.' : '',
  };
}
...
export function buildExamplePrompt(id) {
  ...
  const route = tool.kind === 'API'
    ? `Use the AWB Tools skill. Confirm the API connection and select ${tool.id}. If the connection is missing, explain the setup needed instead of simulating a result.`
    : `Use the AWB book skill. If it is not installed, explain the setup needed. This task uses the book shelf, not the research API.`;
